...
export async function checkAchievementsAction(result: {
  ...
  if (!unlockedIds.has("big_win") && result.totalWin >= result.betAmount * 10)
    toUnlock.push("big_win");
  if (!unlockedIds.has("mega_win") && result.totalWin >= result.betAmount * 50)
    toUnlock.push("mega_win");
  if (!unlockedIds.has("bonus_triggered") && result.bonusTriggered)
    toUnlock.push("bonus_triggered");
  if (!unlockedIds.has("jackpot") && result.jackpotWin && result.jackpotWin > 0)
    toUnlock.push("jackpot");
  ...
  await db
    ...
    .values(toUnlock.map((id) => ({ userId, achievementId: id })))
    .onConflictDoNothing();
...
export async function updateDailyChallengeProgressAction(spinData: {
  ...
  for (const challenge of todayChallenges) {
    ...
    else if (challenge.type === "credits_won" && spinData.creditsWon > 0)
      increment = spinData.creditsWon;
    else if (challenge.type === "games_played") increment = 1;
